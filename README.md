...
| 메시지 조회 성공률 | 69.96% | > 95% | ❌ |
| HTTP 요청 실패율 | 20.82% | - | ❌ |
| 채팅방 조회 95th percentile | 3,187ms | < 1000ms | ❌ |
| 메시지 조회 95th percentile | 2,101.95ms | < 2000ms | ❌ |
...
평균: 649.83ms
중간값: 175.42ms
90th percentile: 2.08s
95th percentile: 2.59s
최대: 5.86s
```

**처리량**
- HTTP 요청수: 33,644개 (61.67 req/s)
- 데이터 수신: 142 MB (259 kB/s)
- 데이터 송신: 22 MB (40 kB/s)

### 심각한 이슈 🚨
1. **높은 실패율**: HTTP 요청의 20.82% 실패
2. **메시지 조회 실패**: 성공률 69.96% (목표 95% 미달)
3. **채팅방 나가기 실패**: 성공률 6% (매우 낮음)
4. **긴 응답 시간**: 최대 5.86초, 평균 649ms
...
### 주요 개선 필요 영역 🔧
1. **데이터베이스 쿼리 최적화**
   - 메시지 조회 성능 저하 (성공률 69.96%)
   - 채팅방 목록 조회 응답시간 초과

2. **부하 분산 및 확장성**
   - 100명 동시 사용자에서 20.82% 요청 실패
   - 응답시간 급격한 증가 (최대 5.86초)

3. **리소스 관리**
   - 채팅방 나가기 기능 심각한 성능 이슈 (6% 성공률)
